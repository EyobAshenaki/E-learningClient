  ...
  <v-dialog v-model="dialog" max-width="500px">
    <template #activator="{ on, attrs }">
      <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">
        Add Teacher Account(s)
      </v-btn>
    </template>
    <v-card v-if="dialog" flat>
      <v-card-title class="primary--text"
        ><span class="text-h5">Add Teacher Account(s)</span></v-card-title
      >
      <v-container fluid>
        <v-radio-group v-model="selectedComponent" row mandatory>
          <v-radio label="Single Input" value="addTeacherForm" />
          <v-radio label="Multiple Input" value="importTeachersForm" />
        </v-radio-group>
      </v-container>
      <component
